...
class UnitIndex:
        ...
        for unit in UNIT_DATA.values():
            ...
            for stat, value in unit["stats"].items():
                if stat not in self.max_stats:
                    ...
                    self.max_stats[stat] = value if value > self.max_stats[stat] else self.max_stats[stat]  # keep the highest value
    ...
    def display_main(self):
        ...
        stats_rect = pr.Rectangle(sides["left"], hp_bar.y + hp_bar.height + 60, hp_bar.width, info_height - 40)
        # pr.draw_rectangle_rec(stats_rect, pr.PURPLE)
        pr.draw_text("STATS RANK:", int(sides["left"]), int(hp_bar.y + hp_bar.height + 25), 20, pr.WHITE)
        ...
        for item, (stat, value) in enumerate(base_stats.items()):
            ...
            # text
            pr.draw_text(f"{stat}: {value}", int(sides["left"]), int(stats_rect.y + stat_height * item + 5), 18, pr.WHITE)

            # stats_bar
            stats_bar = pr.Rectangle(sides["left"], int(stats_rect.y + stat_height * item + 25), stats_rect.width * 0.6, stat_height)
            draw_bar(stats_bar.x, stats_bar.y, stats_bar.width, stats_bar.height * 0.1, (value / self.max_stats[stat]), pr.WHITE, pr.BLACK)

        # talents
        pr.draw_text("TALENTS:", int(sides["right"]), int(hp_bar.y + hp_bar.height + 25), 20, pr.WHITE)
        talent_rect = pr.Rectangle(sides["right"], hp_bar.y + hp_bar.height + 60, hp_bar.width * 0.8, info_height - 40)
